...
| 端口占用 | 8321 被占自动顺延（最多 256 个端口 8321~8576）。启动失败时 Console 警告会**区分三种原因并给排查命令**：① 全部被占（`netstat`/`taskkill` 结束占用进程，检查 Hyper-V/WSL2/Docker 保留区 `netsh int ipv4 show excludedportrange protocol=tcp`）；② 共享冲突/已监听（ErrorCode 32/400，多为多 Unity 实例或同进程残留 listener，关多余实例或重启编辑器）；③ 非端口占用错误（权限/系统资源问题，换端口无效，附官方文档链接） |
